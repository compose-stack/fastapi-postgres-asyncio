...
from .database.postgres import UsersDB
...
async def create_user(db: UsersDB, user: UserCreate):
...
async def verify_user_from_token(
    db: UsersDB, token: str, registration_code: str
) -> bool:
    try:
        data = decode_jwt(
            token,
...
async def activate_user(db: UsersDB, email: str) -> Optional[UserRead]:
    user = await db.get_user_by_email(email)
    if not user:
...
async def increment_failed_login_count(db: UsersDB, email: str):
...
async def authenticate_user(
    db: UsersDB, email: str, password: str
) -> Optional[UserRead]:
    try:
        user = await db.get_user_by_email(email)
        if not user:
...
async def change_user_password(db: UsersDB, user: User, password: str):
    if not user:
        raise exceptions.UserDoesNotExists()
