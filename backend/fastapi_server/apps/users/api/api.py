...
from ..database.postgres import UsersDB, get_user_db
from ..schemas import (
    UserCreate,
    UserRead,
    UserPasswordReset,
...
async def get_user(
    db: UsersDB = Depends(get_user_db),
...
async def request_verify_token(
    email: EmailStr = Body(..., embed=True), db: UsersDB = Depends(get_user_db)
):
...
async def refresh_tokens(
    token: str = Body(..., embed=True), db: UsersDB = Depends(get_user_db)
):
    try:
...
async def forgot_password(
    email: str = Body(..., embed=True), db: UsersDB = Depends(get_user_db)
):
    settings = get_settings()
...
async def reset_password(
    parameters: UserPasswordReset, db: UsersDB = Depends(get_user_db)
):
